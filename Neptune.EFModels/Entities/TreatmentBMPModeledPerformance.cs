...
public static class TreatmentBMPModeledPerformance
{
    /// <summary>
    /// NPT-1068: per-BMP modeled performance for the SPA treatment-bmp-detail page. Mirrors the
    /// shape of <see cref="WaterQualityManagementPlanModeledPerformance.GetModeledPerformanceAsync"/>'s
    /// Detailed-approach branch, just keyed on a single TreatmentBMPID. Returns null if Nereid
    /// hasn't produced a non-baseline result for this BMP yet.
    /// </summary>
    public static async Task<ProjectLoadReducingResultDto?> GetByBMPIDAsync(NeptuneDbContext dbContext, int treatmentBMPID)
    {
        var nereidResults = await dbContext.vLoadReducingResults.AsNoTracking()
            .Where(x => x.TreatmentBMPID == treatmentBMPID && !x.IsBaselineCondition)
            .ToListAsync();
        ...
        // Combined dry weather (summer + winter)
